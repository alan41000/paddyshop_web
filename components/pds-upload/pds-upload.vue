 ...
 */
import { Base64 } from '@/common/libs/base64.js';
...
export default {
	...
	methods: {
		...
		async uploadFile(index = 0) {
			...
			if(this.isOss){
				//===========生成签名===========
				let policy = await ossUtil._getPolicy()
				let signature = await ossUtil._getSignature(this.ossAuthData.accessKeySecret,policy)
				...
				let imgName = this.random_string();
				// console.log(this.lists[index])
				//#ifdef MP-WEIXIN
				let imageType = '.'+this.lists[index].url.split('.')[this.lists[index].url.split('.').length-1]				
				//#endif
				//#ifdef H5
				let imageType = '.'+this.lists[index].file.name.split('.')[this.lists[index].file.name.split('.').length-1]	
				//#endif
				// 生成日期文件夹 自定义，可根据自身业务 区分文件夹。每个 ‘ / ’ 即是一级目录
				let dir = this.ossFile + year + (month < 10 ? '0' + month : month) + (day < 10 ? '0' + day : day) + '/'
				let ossPath = dir + imgName + imageType
				...
				const task = uni.uploadFile({
					...
					formData: {
						'key': ossPath, 
						'policy': policy,
						'OSSAccessKeyId': this.ossAuthData.accessKeyId,
						'success_action_status': '200', //让服务端返回200,不然，成功上传图片后阿里默认返回statusCode: 204
						'x-oss-security-token': this.ossAuthData.securityToken,
						'signature': signature,
					},
					header: this.header,
					success: res => {
						...
						if (![200, 201, 204].includes(res.statusCode)) {
							...
							this.lists[index].error = false;
							this.lists[index].imgSrc = ossPath;
